...
| 位置 | 文件 | 现值 | 影响 |
|---|---|---|---|
| PC + 前端 | package.json → version | 1.7.5 | 安装包命名、latest.yml version、electron-updater 比较基准；vite 构建时注入 __APP_VERSION__（vite.config.ts）→ 前端 LOCAL_VERSION |
| Android | android/app/build.gradle → defaultConfig | versionName 1.7.5 / versionCode 46 | APK 版本；Android 应用内更新比较的 LOCAL_VERSION（原生 versionName 优先） |
