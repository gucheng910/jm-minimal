...
| `driver-scrollleak.js` | 列表位置记忆不许被污染：**进详情返回后，列表必须回到离开时的 window.scrollY**。守的是 2026-09-24 那个"又出现的老 bug"——`openDetail` 里 `if (from === "list") saveScrollTarget(window.scrollY)` 缺 mode 守卫，而详情页的「相关漫画」也走 `from="list"`，于是**详情页的滚动量被当成列表位置存了起来**，返回时列表被滚到那个值（看起来像"外面的列表自己滚动"）。用例对比 H2 正常返回 与 H1 经「相关漫画」返回，并断言详情页里 `.list-item` 不在 DOM（排除"后面还挂着一份列表"这条解释） |
