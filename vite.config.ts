 ...
 *                           老内核由 @vitejs/plugin-legacy 的现代性探测脚本自动加载
 *
 * 2.1.0 起原来的 legacy（Android 6 / minSdk 23 / 关 ServiceWorker / 不含去条纹）**并入 compat**：
 * 同一个 --mode compat 产物，加上 JM_NO_SEAM=1 / JM_NO_SW=1 / minSdk 23 就是老安卓包，
 * 构建入口是 tools/build-compat-apk.ps1（原来那份 build-legacy-apk.ps1 已合并进来）。
 */
export default defineConfig(({ mode }) => {
  const compat = mode === "compat";
  /** 需要 ES5 legacy 产物的那一档（老内核 / 老安卓包） */
  const oldKernel = compat;
  const variant = compat ? "compat" : "modern";
  return {
    plugins: [
      react(),
      ...(oldKernel
    ...
    define: {
      ...
      // 构建变体：更新器据此挑选对应的 APK 资产（两个包同名同版本，只能构建期区分）
      __BUILD_VARIANT__: JSON.stringify(variant),
      // 老安卓构建（build-compat-apk.ps1 里 JM_NO_SEAM=1）：整块去条纹（接缝修复）逻辑被摇掉，
